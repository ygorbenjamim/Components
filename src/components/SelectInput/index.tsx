...
const SelectInput = ({
  ...
  const handlePressDateTime = useCallback(
    (event: Event, selectedDateTime: Date | undefined) => {
      ...
      if (type == 'date')
        setFormatDateTime(moment(selectedDateTime).format('DD/MM/YYYY'));
      ...
      <ContentSelectInput design={design}>
        {label && (
          <Label>
            {label}{' '}
            {requiredLabel && (
              <MCIcon name="asterisk" size={13} color={colors.error} />
            )}
          </Label>
        )}
        <ContentRow>
          ...
          <ContainerModal>
            ...
            <FlatList
              ...
              renderItem={({item, index}: IFlatListProps) => (
                <Item onPress={() => handlePressText(item)} value={item} />
              )}
